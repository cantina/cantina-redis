...
function RedisCollection(options) {
  ...
  // attached.
  this.model = options.model || RedisModel;
  if (!this.model.all) {
    this.model = createModel(this.model, { client: this.client });
  }
  else if (!this.model.client) {
    this.model.client = this.client;
  }

  this.schema = RedisModel.super.getSchema(this.model);

  RedisCollection.super.call(this);
}
inherits(RedisCollection, EventEmitter);
