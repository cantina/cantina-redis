...
function RedisModel(attrs, options) {
  ...
  this.on('save:after', function() {
    self._hydrate();
    self.client.SADD(self.prefixKey(), self.id);
  ...
  this.on('destroy:after', function() {
    self.client.SREM(self.prefixKey(), self.id);
    self.indexes.forEach(function(index) {
      if (typeof self.properties[index] != 'undefined') {
...
/**
 * Load a model from redis, replacing all existing properties with those
 * stored in the database.
 *
 * Example
 * =======
 *
 *     // Given a model class 'Fruit'.
 *     var apple = new Fruit({id: 'xFh8Dh8a'});
 *     apple.load(function(err) {
 *       // apple's properties now match those found in the database.
 *     });
 *
 * @method load
 * @param cb {Function} Callback to handle errors and act on updated model.
 */
RedisModel.prototype.load = function(cb) {
  var self = this;
  this.client.HGETALL(self.prefixKey(this.id), function(err, obj) {
    if (err) {
      return cb(err);
    }
    if (!obj) {
