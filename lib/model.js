...
RedisModel.prototype.init = function(attrs, options) {
  ...
  this.on('save:after', function() {
    ...
    this.indexes.forEach(function(index) {
      self.client.SADD(prefixId(index + ':' + JSON.stringify(self.properties[index]), self.namespace), self.id);
    });
  });
  this.on('destroy:after', function() {
    ...
    var self = this;
    this.indexes.forEach(function(index) {
      self.client.SREM(prefixId(index + ':' + JSON.stringify(self.properties[index]), self.namespace), self.id);
    });
  });
