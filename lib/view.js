...
var utils = require('cantina-utils'),
    ...
    lib = require('../'),
    getSchema = lib.RedisModel.super.getSchema;
...
function RedisView(options) {
  ...
  this.models.forEach(function(model, i) {
    if (!model.all) {
      self.models[i] = model = lib.createModel(model, {client: self.client});
    }
...
RedisView.prototype.list = function(limit, skip, dir, callback) {
  ...
  if (this.cache && (dir === this.dir) && (skip === 0 || ((limit + skip) <= this._cache.length))) {
...
RedisView.prototype.repopulate = function(callback) {
  ...
  this.destroy(function(err) {
    ...
    self.models.forEach(function(model) {
      var col = new lib.RedisCollection({model: model});
      tasks.push(function(done) {
        col.find({}, function(err, models) {
          if (err) return done(err);
          models.forEach(function(model) {
            self.afterSave(model, done);
          });
        });
      });
    });
